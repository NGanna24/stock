...
class Facture {
    ...
    static async findAll(filters = {}, id_utilisateur) {
        ...
        try {
            ...
            `;
            // ⚠️ Ordre des params : 2 sous-requêtes + WHERE
            const params = [id_utilisateur, id_utilisateur, id_utilisateur];

            if (filters.search) {
                query += ` AND (f.numero_facture LIKE ?
            ...
            query += ' ORDER BY f.date_facture DESC, f.id_facture DESC';
        ...
        try {
            ...
            const [rows] = await pool.execute(
                `SELECT f.*,
                 ...
                 LEFT JOIN utilisateurs u ON f.id_utilisateur = u.id_utilisateur
                 WHERE f.id_facture = ?
                   AND f.id_utilisateur = ?`,        // ✅ ISOLATION
        ...
        try {
            const [result] = await pool.execute(
                `UPDATE factures_vente SET statut = ?
                 WHERE id_facture = ? AND id_utilisateur = ?`,   // ✅ ISOLATION
    ...
    /**
     * ============================================================
     * Statistiques des factures (par workspace)
     * ============================================================
     */
        ...
        try {
            const [rows] = await pool.execute(
                `SELECT
                    ...
                    SUM(CASE WHEN statut = 'partiellement_payee' THEN 1 ELSE 0 END) as partiellement_payee,
                    SUM(CASE WHEN statut = 'en_retard'          THEN 1 ELSE 0 END) as en_retard,
                    SUM(CASE WHEN statut = 'annulee'            THEN 1 ELSE 0 END) as annulee,
                    COALESCE(SUM(CASE WHEN statut IN ('en_attente', 'partiellement_payee')
                                     THEN montant_total ELSE 0 END), 0) as montant_impaye,
            ...
            return {
                ...
                payee: parseInt(stats.payee) || 0,
                partiellement_payee: parseInt(stats.partiellement_payee) || 0,
                en_retard: parseInt(stats.en_retard) || 0,
